...
// The LogQL parser pulls in dskit's memberlist client, which needs Grafana's
// fork of hashicorp/memberlist. Loki replaces it in its own go.mod, and replace
// directives are not inherited by consumers, so it has to be repeated here.
